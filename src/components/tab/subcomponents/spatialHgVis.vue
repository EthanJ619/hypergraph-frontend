...
export default {
  ...
  computed: {
    resultImgUrl() {
      ...
      // return "http://172.16.8.24:20120/software6/api/static/" + this.m_taskName;   //山东部署
    },
  },
  watch: {
    provCode() {
  ...
  methods: {
      ...
      this.disProvince = new AMap.DistrictLayer.Province({
        ...
        styles: {
          fill: (properties) => {
            ...
            // adcode_cit
            // adcode
            return this.getProvColor(properties.adcode);
          },
    ...
    putNationMap() {
      this.disCountry = new AMap.DistrictLayer.Country({
        ...
        styles: {
          ...
          fill: (props) => {
            return this.getNationColor(props.adcode_pro);
          },
        },
      });
      ...
      this.disCountry.on("mouseover", (e) => {
        this.disCountry.setStyles({
          fill: (props) => {
            ...
            //中国特有字段
            return this.getNationColor(props.adcode_pro, e.props.adcode_pro);
          },
          "nation-stroke": "#ff0000",
          "coastline-stroke": [0.85, 0.63, 0.94, 1],
          "province-stroke": "white",
    ...
    getNationColor(adcode) {
      if (!this.nationColors[adcode]) {
        var sampleN = this.sampleNum[adcode];
        if (!sampleN) {
          this.nationColors[adcode] = "rgb(235,235,235)";
        } else {
          var rg = 235 - Math.floor(((sampleN - 5) / 5) * 235);
          this.nationColors[adcode] = "rgb(235," + rg + "," + rg + ")";
        }
      }
    ...
    getProvColor(adcode) {
      if (!this.provColors[adcode]) {
        var sampleN = this.sampleNum[adcode];
        if (!sampleN) {
          this.provColors[adcode] = "rgb(235,235,235)";
        } else {
          var rg = 235 - Math.floor(((sampleN - 5) / 5) * 255);
          this.provColors[adcode] = "rgb(" + rg + ",235," + rg + ")";
        }
      }
      return this.provColors[adcode];
    },
      ...
      if (e.props.adcode_pro == this.provCode) {
        this.disProvince.setStyles({
          fill: (props) => {
            if (e.props.adcode == props.adcode) return "rgba(240, 162, 20, 1)";
            //中国特有字段
            return this.getProvColor(props.adcode);
          },
          "province-stroke": "cornflowerblue",
          "city-stroke": "white", // 中国地级市边界
          "county-stroke": "rgba(255,255,255,0.5)", // 中国区县边界
        ...
        this.infoWindow.setContent(
          // this.customInfoWindow(e.props.NAME_CHN, "病例数：---<br/>占比：--%")
          this.customInfoWindow(e.props.NAME_CHN, "病例数：5192<br/>占比：52.26%")  //绵阳特供
        );
