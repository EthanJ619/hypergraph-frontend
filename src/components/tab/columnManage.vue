...
import { getTableData } from "@/utils/tableDescribe.js";
import { getFeatures } from "@/utils/feature.js";
import { addDisease, removeCate } from "@/utils/category";
...
export default {
  ...
  methods: {
    ...
    append() {
      ...
      for (let j = 0; j < this.checkedCharacter_Arr.length; j++) {
        for (let i = 0; i < this.checkedCharacter_Arr[j].length; i++) {
          this.fields[k] = this.checkedCharacter_Arr[j][i].featureName;
          k++;
        }
      }
      console.log("特征列表:");
      console.log(this.fields);
      postRequest("/field/updateField", {
      ...
      addDisease("/api/addFeature", catagoryNodeJSON)
        .then((response) => {
          this.getCategory(); //刷新目录结构
    ...
    appendDisease() {
      ...
      let catagoryNodeJSON = JSON.stringify(catagoryNode);
      addDisease("/api/appendDisease", catagoryNodeJSON)
        .then((response) => {
          this.getCategory(); //刷新目录结构
          console.log(response.data);
        })
    ...
    addCharacteration() {
      getFeatures("/api/feature/getFeatures", 0)
        .then((response) => {
          console.log("特征为：");
          console.log(response.data);
      ...
      getFeatures("/api/feature/getFeatures", 2)
        .then((response) => {
          console.log("特征为：");
          console.log(response.data);
      ...
      getFeatures("/api/feature/getFeatures", 3)
        .then((response) => {
          console.log("特征为：");
          console.log(response.data);
    ...
    getTableData(tableId, tableName) {
      getTableData("/api/getTable", tableId, tableName)
        .then((response) => {
          // 获取表数据
    ...
    changeData(...theArgs) {
      ...
      if (theArgs[0].isLeafs == 1) {
        postRequest("/field/getAllField", { diseaseName: disease }).then(
          (res) => {
            console.log(res);
